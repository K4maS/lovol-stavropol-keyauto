...
class sinobyMail
{
	public static $mailConfig = array(
		...
		"smtp"=>array(
			"login" => "uaz.ng@yandex.ru",
			"password" => "gbuttyhrowtcrbzm",
			"server" => "smtp.yandex.ru",
			"port" => "465"
		),

		"from" 	=> "uaz.ng@yandex.ru",

		"to"	=> "sinobyprog@yandex.ru, g.shumakov@sinoby.ru",
...
EOF
	...
	public static function init()
	{
		...
		//Хост без WWW
		self::templateSet( "hostWithoutWWW", str_replace( "www.", "", $_SERVER["HTTP_HOST"] ) );
		//Адрес сайта
		...
		sinobyMail::templateSet( "spam", "" );

		if ( !WAF::isSendSuccess( $output ) ) {
			$error = $output["error"];
			self::templateSet( "spam", " <div style='color: white;background-color: hsl(0, 50%, 50%); padding:20px;'>Возможно спам<br>".$error."</div> " );
		}
	}//end_ func
	...
	static function SendMailSMTP( $input )
	{
		...
		$mail->isSMTP();
		$mail->SMTPAuth = true;
		$mail->Host = trim($smtp["server"]);
		$mail->Port = trim($smtp["port"]);
		$mail->Username = trim($smtp["login"]);
		$mail->Password = trim($smtp["password"]);
...
exit;*/
		...
		$mail->AddCustomHeader("List-Unsubscribe: <mailto:unsubscribe@".$_SERVER['HTTP_HOST']."?subject=Unsubscribe>, <".$l_unsubscribe_url.">");
       		if (!$mail->send())
		{
			echo "<pre>";
			echo "SMTP ERROR".$mail->ErrorInfo;exit;
		}//end_ func
		return 1;
	}//end_ func
